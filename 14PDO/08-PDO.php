...
//query方法：PDO::query — 执行 SQL 语句，以 PDOStatement 对象形式返回结果集，查询对象
//$sql = "SELECT * FROM news_list ORDER BY id DESC LIMIT 5";
//$pdoStatment = $pdo->query($sql); // 返回的是一个PDOStatement的结果集

// lastInsertId 方法：PDO::lastInsertId ([ string $name = NULL ] ) : string
// 返回最后插入行的ID值，或者是一个序列对象最后的值。
